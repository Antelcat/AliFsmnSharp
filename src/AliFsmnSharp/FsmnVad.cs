...
public sealed class FsmnVad : IDisposable
{
    ...
    public FsmnVad(FsmnVadModel model,  FsmnVadOptions? options = null)
    {
        options ??= new();
        ...
        session = new InferenceSession(model.Model, sessionOptions);
        var vadYamlEntity = YamlHelper.ReadYamlText<VadYamlEntity>(model.Yaml);
        SampleRate    = vadYamlEntity.frontend_conf.fs;
        frontend      = new WavFrontend(model.Mvn.Split('\n'), vadYamlEntity.frontend_conf);
    ...
    {
        var (feats, featsCount) = frontend.ExtractFeat([samples]);
